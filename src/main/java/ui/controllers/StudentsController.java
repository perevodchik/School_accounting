...
public class StudentsController {
    ...
    protected void initialize() {
        ...
        schoolClassDao.getAll().forEach(s -> schoolClassList.getItems().add((SchoolClass) s));
    ...
    private void handleButtonClicks(javafx.event.ActionEvent mouseEvent) {
        if (mouseEvent.getSource() == addStudent) {
            ...
            StudentDao studentDao = new StudentDao();
            ObservableList<Student> list = studentTable.getSelectionModel().getSelectedItems();
            if(list.size() > 0) list.forEach(studentDao::delete);
            setTableData();
        } else if (mouseEvent.getSource() == changeStudent) {
            if(studentTable.getSelectionModel().getSelectedItems().size() == 1) new ChangeStudentModal(studentTable.getSelectionModel().getSelectedItem()).showModal();
        }
    }
}
