...
public class ZhurnalController {
    ...
    @FXML
    public JFXCheckBox MissCB;
    @FXML
    public JFXCheckBox HealCB;
    @FXML
    public JFXCheckBox ContrCB;
    @FXML
    public JFXCheckBox SelfCB;
    @FXML
    public JFXCheckBox HomeCB;
    @FXML
    public JFXCheckBox ClassCB;
    ...
    private void getPoints() {
        ...
        for(Student student: studentList)
        {
            ...
            for(int c = 1; c < tableView.getColumns().size(); c++)
            {
                ...
                ArrayList<String> selectedReason = new ArrayList<>();
                if(ClassCB.isSelected())  selectedReason.add("Класне заняття");
                if(HomeCB.isSelected())  selectedReason.add("Домашнє завдання");
                if(SelfCB.isSelected())  selectedReason.add("Самостійна робота");
                if(ContrCB.isSelected())  selectedReason.add("Контрольна робота");
                if(HealCB.isSelected())  selectedReason.add("Хворів");
                if(MissCB.isSelected())  selectedReason.add("Відсутній");
                System.out.println(selectedReason);

                pList = pList.stream().filter(p -> selectedReason.contains(p.getReason().getName())).collect(Collectors.toList());
    ...
    private void showModal() {
        ...
        new AddPointModal(p, currentMonth).showModal();
    }

    static ZhurnalController getInstance() {
        return INSTANCE;
    }
